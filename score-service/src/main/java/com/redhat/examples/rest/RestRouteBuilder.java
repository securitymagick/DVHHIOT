...
public class RestRouteBuilder extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        rest("/provider").description("User Score rest service")
            .consumes("application/json").produces("application/json")

            .get("/{username}").description("Find provider by username").outType(Provider.class)
                .to("bean:providerService?method=getProvider(${header.username})")

            .put("/").description("Updates a user's score").type(Provider.class)
                .to("bean:providerService?method=updateProvider")

            .get().description("List all users and scores").outTypeList(Provider.class)
                .to("bean:providerService?method=listProviders");
				
            //.get("/{username}/create").description("Creates a user").type(Provider.class)
            //    .to("bean:providerService?method=createProvider(${header.username})");               
    }
